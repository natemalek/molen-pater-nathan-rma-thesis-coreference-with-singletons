''' Extract the result of cort models on the right corpus

Usage:
    extract_results.py --out=<p> <log_path>...
'''
import os
import re
...
def extract_data(log_path):
    with open(log_path) as f:
        content = f.read()
    info_m = re.match(r'Evaluating ([\w\-]+) on .+(sample_\d+)/((?:no-)?[\w\d_]+)/(dev_test)\.m_(auto|gold)_conll', content)
    print('File:', log_path)
    print('Extracted info:', info_m.groups())
    return {
        'system': 'cort-' + info_m.group(1),
